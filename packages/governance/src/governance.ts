 ...
 * The kernel factory.
 *
 * `createGovernanceKernel()` builds an instance with the policy engine, the
 * audit chain, the storage contract, the session ledger and the plugin
 * registry — and nothing that belongs to a plugin. It knows no detector, no
 * standards mapping and no scoring corpus.
 *
 * `createGovernance()` in the package barrel is this plus the default
 * extension set, which is what preserves the public API: a caller who has
 * never heard of the plugin contract still gets injection detection and
 * sensitive-data masking exactly as before.
 *
 * The split is what lets the kernel ship as a package that does not depend on
...
/**
 * Behaviour the kernel does not carry itself, applied synchronously at
 * construction.
 *
 * Deliberately not the async plugin contract: these must exist before the
 * first `enforce()` for the documented behaviour of the built-in conditions
 * to hold, so they cannot wait on a promise. Anything that can wait should be
 * a `GovernancePlugin` and go through `gov.use()`.
 */
export interface KernelExtensions {
  /** Condition types to register — e.g. the detector's `injection_guard`. */
...
export function storedToRegistration(agent: StoredAgent): AgentRegistration {
 ...
 * ```
 */
export function createGovernanceKernel(config: GovernanceConfig = {}): GovernanceInstance {
  ...
  const policies = createPolicyEngine({
    ...
    // Extension conditions register before the caller's, so a caller can
    // still override one by name with `conditions`.
    conditions: [...(extensions.conditions ?? []), ...(config.conditions ?? [])],
  });
